...
- `assets/img/deities.png` — the artwork at the top of the invitation. Supplied by the couple; the
  flat background was keyed out so it sits directly on the cream page, then palette-quantised
  (617 × 466, 87 KB).
- `assets/img/couple-haldi.jpg` — the bride-and-groom image in the hero. Supplied by the couple
  (360 × 360, 35 KB).
...
Only use images you own or are licensed to use; this page is public once deployed. The two images
currently in `assets/img/` were supplied by the couple — if either came from a stock library, check
the licence covers use on a public website (and add attribution if the licence requires it).

### Higher-resolution originals

The supplied images are small: 617 × 466 and 360 × 360. They look fine at the sizes used here, but
they are close to their limit — the couple image in particular is displayed at 330 px, so on a
high-density phone screen it is doing about 1.1× rather than the 2× that would look perfectly crisp.
If larger originals exist, drop them in over the top and nothing else needs to change.
