...
After the opening tap, three scenes play before the invitation appears (about 20 seconds):

**1 · Where our story began** (13s, one unbroken camera move)
A midnight-blue field of drifting golden motes. A gold line draws the outline of Kansas, towns light
up across it one by one, and one light — Kansas City, at the notch in the north-east — grows brighter
than the rest. The camera dives into it and the light opens out into a Kansas City evening: skyline,
lit windows, street lamps, bokeh. The two of them stand together in silhouette, a golden thread
between them, warm air drifting past. *We met in Kansas…* An elegant mandap appears in the distance,
marigolds along its arch, diyas at its feet, fairy lights above, with the outline of Kansas faint
behind it and the thread running from where they met to where they marry. As they walk on, their
silhouettes change — a turban, a veil. The scene dims and settles on
*…and in Kansas, our forever begins. / Teja ♥ Deeksha / October 21, 2026 · Kansas City.*
...
**3 · The first selfie** (3.6s) — a viewfinder frames the shot, the shutter taps, and a white flash
hands over to the invitation.

Scene 1 is a single SVG under one `.s1-cam` group, so the whole thing is one continuous camera move
with no cuts — the layers cross-fade beneath it. The Kansas outline measures its own path at runtime
(`getTotalLength()`) and sets the dash from that, so editing the shape never breaks the draw-on.
Motes, town lights, skyline windows, street lamps, bokeh, marigolds, diyas and fairy lights are all
generated by `seedScene()` rather than hand-placed.
...
so it can never run out of sync with the music. There is a **Skip** button throughout — at 20 seconds
it matters — and the sequence is bypassed entirely for anyone whose device asks for reduced motion.
